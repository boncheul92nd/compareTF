...
def unwrap_tf(phase_angle, discont=np.pi, axis=-1):
    dd = diff_tf(phase_angle, axis=axis)
...
def instantaneous_frequency_tf(phase_angle, time_axis=-2):
    phase_unwrapped = unwrap_tf(phase_angle, axis=time_axis)
    dphase = diff_tf(phase_unwrapped, axis=time_axis)
...
if __name__ == "__main__":
    ...
    tensor_unwrapped = unwrap_tf(tensor_phase, axis=-1)
    tensor_instntfreq = instantaneous_frequency_tf(tensor_phase)
